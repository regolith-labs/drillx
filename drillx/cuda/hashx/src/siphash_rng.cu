...
__host__ void hashx_siphash_rng_init(siphash_rng* gen, const siphash_state* state) {
	gen->keys = *state;
	gen->counter = 0;
	gen->count8 = 0;
	gen->count32 = 0;
...
__host__ uint8_t hashx_siphash_rng_u8(siphash_rng* gen) {
	if (gen->count8 == 0) {
...
__host__ uint32_t hashx_siphash_rng_u32(siphash_rng* gen) {
	if (gen->count32 == 0) {
